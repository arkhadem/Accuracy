...
def fc_layer_extractor(writer, net_data, net_data_v1, net_data_v2, layer_name):
	...
	T_output_num = T_output_size * T_output_size;
	...
	encoded_modified_index_1d_file = open(layer_name + "_index_1d_tiled_modified_enc.txt", "w")
				...
				for idx_number in range(0, len(modified_indices[line_number])):
					...
					if(modified_indices[line_number][idx_number] == -1):
						...
						if larger_than_delta or new_weight or idx_is_zero:
							# idx is absolute
							idx_memory_size += (2 + log2(T_output_num))
							to_write = ((modified_indices[line_number][idx_number] & (T_output_num - 1)) << 2) + 3
							write_enc_idx(to_write, 2 + log2(T_output_num), encoded_modified_index_1d_file)
						else:
							# idx is delta
